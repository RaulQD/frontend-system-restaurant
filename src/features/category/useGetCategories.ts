import {  getCategoriesPagination } from '@/services/apiCategory';
...
export const useGetCategories = () => {
  ...
  const { data: categories, isLoading, isError, error } = useQuery({
    ...
    queryFn: () => getCategoriesPagination({ keyword, page }),
    retry: false,
  })
  ...
  if (page < pageCount) {
    queryClient.prefetchQuery({
      ...
      queryFn: () => getCategoriesPagination({ keyword, page: page + 1 }),
    })
